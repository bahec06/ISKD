...
NiFpga_Status qt_ni_fpga::fpga_init(uint16_t *spectrum, uint16_t *pulse_form) {
    ...
    fpga_stat = NiFpga_ConfigureFifo(fpga_sess, NiFpga_fpga_main_HostToTargetFifoI16_const_fifo, const_fifo_depth);
    if(fpga_stat != NiFpga_Status_Success) {
        fpga_p2p_endpoint = 0;
        return fpga_stat;
    }
...
NiFpga_Status qt_ni_fpga::fpga_reinit_forms(uint16_t *spectrum, uint16_t *pulse_form) {
    fpga_stat = NiFpga_WriteFifoU16(fpga_sess, NiFpga_fpga_main_HostToTargetFifoU16_config_fifo, spectrum, SPEC_SIZE, 5000, &config_fifo_elements); //Записать спектр в FPGA
    if(fpga_stat != NiFpga_Status_Success) {
        fpga_p2p_endpoint = 0;
        return fpga_stat;
...
NiFpga_Status qt_ni_fpga::fpga_close() {
    fpga_stat = NiFpga_StopFifo(fpga_sess, NiFpga_fpga_main_HostToTargetFifoI16_host_pnum_fifo);
    if(fpga_stat != NiFpga_Status_Success) {
        return fpga_stat;
    }

    fpga_stat = NiFpga_StopFifo(fpga_sess, NiFpga_fpga_main_HostToTargetFifoI16_const_fifo);
    if(fpga_stat != NiFpga_Status_Success) {
        return fpga_stat;
    }
...
//Остановка FPGA
NiFpga_Status qt_ni_fpga::fpga_stop() {
...
void qt_ni_fpga::read_freq() {
    fpga_stat = NiFpga_ReadI16(fpga_sess, NiFpga_fpga_main_IndicatorI16_voltage_ch1, &voltage_ch1);
        if(fpga_stat != NiFpga_Status_Success) {
            return;
        }
}
...
void qt_ni_fpga::write_fifo(int16_t *buffer, size_t length, size_t *empty_elements){
    fpga_stat = NiFpga_WriteFifoI16(fpga_sess, NiFpga_fpga_main_HostToTargetFifoI16_host_pnum_fifo, buffer, length, 1000, empty_elements);
    if(fpga_stat != NiFpga_Status_Success) {
        return;
    }
}

void qt_ni_fpga::write_const_fifo(int16_t *buffer, size_t length, size_t *empty_elements){
    fpga_stat = NiFpga_WriteFifoI16(fpga_sess, NiFpga_fpga_main_HostToTargetFifoI16_const_fifo, buffer, length, 1000, empty_elements);
    if(fpga_stat != NiFpga_Status_Success) {
        return;
    }
}
